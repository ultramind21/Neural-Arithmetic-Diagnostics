...
## Appendix: Validation terminology

**Reproduction checks vs policy checks.** Project 12 distinguishes two validation modes based on procedure lineage. A *reproduction check* compares Project 12 artifacts against historical baseline artifacts under a strict numeric tolerance; this mode applies when procedures are deterministic (e.g., re-running V3.1 evaluation). A *policy check* directly evaluates whether pre-registered criteria are satisfied on a recorded artifact; this mode applies when procedures are stochastic (e.g., training runs). In Project 11 (Section 3.2), C01 validation uses strict repro checks: V3.1 macroF1_present is compared within tolerance to historical runs. In Project 4 (Section 4.2), C04 validation uses policy checks: the artifact is evaluated for seen+held-out gap ≥ threshold, without requiring numeric match to prior training runs. Finite-seed smoke checks (e.g., 3-seed P4 sweep) can supplement policy checks to test consistency across random seeds while accepting stochastic variance.
