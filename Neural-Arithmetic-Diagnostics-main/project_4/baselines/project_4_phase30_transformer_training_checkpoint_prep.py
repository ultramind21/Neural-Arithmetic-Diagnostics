"""
================================================================================
PROJECT 4 PHASE-30 TRANSFORMER TRAINING CHECKPOINT PREP
================================================================================

PURPOSE:
  Prepare a trained Transformer checkpoint from phase_30_multidigit_learning.py
  for use in Project 4 baseline evaluation.

ROLE:
  This script isolates the checkpoint-preparation step so that:
    - Transformer baseline evaluation is not run on untrained weights
...
CURRENT SCOPE:
  - TransformerSequenceArithmetic only

OUTPUT:
  project_4/checkpoints/phase30_transformer_project4_ready.pt
...
CHECKPOINT_PATH = CHECKPOINT_DIR / "phase30_transformer_project4_ready.pt"
...
def main():
    print_header("PROJECT 4 PHASE-30 TRANSFORMER TRAINING CHECKPOINT PREP")
    ...
    # ------------------------------------------------------------------------
    print_header("4) BUILD TRANSFORMER MODEL")

    print(f"✓ Building model with max_length={max_length}")
    model = module.TransformerSequenceArithmetic(max_length=max_length).to(device)
    ...
    torch.save(
        {
            ...
            "model_type": "TransformerSequenceArithmetic",
            "max_length": max_length,
            "source_file": str(TARGET_FILE),
            "project4_role": "phase30_transformer_baseline_checkpoint",
        },
        CHECKPOINT_PATH,
    )
    ...
    print("\n" + "=" * 80)
    print("TRANSFORMER CHECKPOINT PREP COMPLETE")
    print("=" * 80)
