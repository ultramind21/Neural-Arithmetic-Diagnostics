...
REPORT_MD = P11_RESULTS / "prediction_gate_v2_report.md"
ARTIFACT_JSON = P11_RESULTS / "prediction_gate_v2_artifact.json"


# =============================================================================
# Project 10 ground truth (exact rule)
# =============================================================================
...
def main():
    ...
    for pt in points:
        ...
        if pid not in pred_by_id:
            raise ValueError(f"Missing prediction for id={pid}")
        p = pred_by_id[pid]
        if float(p["H"]) != float(pt["H"]) or float(p["P"]) != float(pt["P"]):
            raise ValueError(f"H,P mismatch for id={pid}")
    ...
    best_baseline_f1 = max(
        metrics["baseline_majority_from_project10_grid"]["macro_f1"],
        metrics["baseline_nearest_neighbor_project10_grid"]["macro_f1"],
    )

    passed = (metrics["model"]["macro_f1"] >= best_baseline_f1 + 0.15) and (metrics["model"]["macro_f1"] >= 0.80)
    ...
    artifact = {
        ...
        "pass_condition": {
            "required": "macro_f1 >= best_baseline + 0.15 AND macro_f1 >= 0.80",
            "passed": bool(passed)
        },
    ...
    lines = []
    lines.append("# PROJECT 11 — PREDICTION GATE V2 REPORT")
    lines.append("")
    ...
    lines.append(f"- majority label: `{majority_label}`")
    lines.append(f"- majority macro-F1: {metrics['baseline_majority_from_project10_grid']['macro_f1']:.4f}")
    lines.append(f"- nearest-neighbor macro-F1: {metrics['baseline_nearest_neighbor_project10_grid']['macro_f1']:.4f}")
    lines.append("")
