...
SYSTEM_PATH = P11_RESULTS / "transfer_t1_system.json"
HOLDOUT_PATH = P11_RESULTS / "transfer_t1_holdout_points.json"
PRED_PATH = P11_RESULTS / "transfer_t1_predictions.json"

REPORT_MD = P11_RESULTS / "transfer_t1_report.md"
ARTIFACT_JSON = P11_RESULTS / "transfer_t1_artifact.json"
...
def ground_truth_region_t1(system: dict, H: float, P: float) -> CellResult:
    ...
    return CellResult(
        H=H, P=P,
        avg_uni=avg_uni, avg_fam=avg_fam, gap=gap,
...
def normalized_distance(aH: float, aP: float, bH: float, bP: float) -> float:
    H_range = 0.015 - 0.003  # keep same normalization as Project 10
    P_range = 0.42 - 0.30
    return (((aH - bH) / H_range) ** 2 + ((aP - bP) / P_range) ** 2) ** 0.5


def build_t1_grid(system: dict) -> List[Tuple[float, float, str]]:
    Hs = [0.003, 0.009, 0.015]
    Ps = [0.30, 0.36, 0.42]
    grid = []
    for H in Hs:
        for P in Ps:
            grid.append((H, P, ground_truth_region_t1(system, H, P).region))
    return grid
...
def majority_label_from_grid(grid: List[Tuple[float, float, str]]) -> str:
    ...
    for _, _, lbl in grid:
        counts[lbl] += 1
    # tie-break: universal > transition > family-aware (deterministic)
...
def main():
    ...
    for pt in points:
        ...
        gt = ground_truth_region_t1(system, H, P)
    ...
    best_baseline_f1 = max(
        metrics["baseline_majority_from_t1_grid"]["macro_f1"],
        metrics["baseline_nearest_neighbor_t1_grid"]["macro_f1"],
    )
    ...
    lines = []
    lines.append("# PROJECT 11 — TRANSFER T1 REPORT")
    lines.append("")
    ...
    lines.append("")
    lines.append("## T1 grid label counts (3x3)")
    lines.append(f"- {metrics['t1_grid_counts']}")
    lines.append("")
    lines.append("## Baselines")
    lines.append(f"- majority label (from T1 grid): `{majority_lbl}`")
    lines.append(f"- majority macro-F1: {metrics['baseline_majority_from_t1_grid']['macro_f1']:.4f}")
    lines.append(f"- nearest-neighbor macro-F1: {metrics['baseline_nearest_neighbor_t1_grid']['macro_f1']:.4f}")
    lines.append("")
