...
HOLDOUT_PATH = P11_RESULTS / "transfer_t4_holdout_points.json"
PRED_PATH = P11_RESULTS / "transfer_t4_predictions.json"

REPORT_MD = P11_RESULTS / "transfer_t4_report.md"
ARTIFACT_JSON = P11_RESULTS / "transfer_t4_artifact.json"
...
def main():
    ...
    best_baseline_f1 = max(
        metrics["baseline_majority_from_t4_grid"]["macro_f1"],
        metrics["baseline_nearest_neighbor_t4_grid"]["macro_f1"],
    )
    ...
    lines = []
    lines.append("# PROJECT 11 — TRANSFER T4 REPORT (Rule V3)")
    lines.append("")
    ...
    lines.append("")
    lines.append("## T4 grid label counts (3x3)")
    lines.append(f"- {metrics['t4_grid_counts']}")
    lines.append("")
    lines.append("## Baselines")
    lines.append(f"- majority label (from T4 grid): `{majority_lbl}`")
    lines.append(f"- majority macro-F1: {metrics['baseline_majority_from_t4_grid']['macro_f1']:.4f}")
    lines.append(f"- nearest-neighbor macro-F1: {metrics['baseline_nearest_neighbor_t4_grid']['macro_f1']:.4f}")
    lines.append("")
