...
SYSTEM_PATH = P11_RESULTS / "transfer_t2_system.json"
HOLDOUT_PATH = P11_RESULTS / "transfer_t2_holdout_points.json"
PRED_PATH = P11_RESULTS / "transfer_t2_predictions.json"

REPORT_MD = P11_RESULTS / "transfer_t2_report.md"
ARTIFACT_JSON = P11_RESULTS / "transfer_t2_artifact.json"
...
def normalized_distance(aH: float, aP: float, bH: float, bP: float) -> float:
    # keep same normalization as Project 10 for consistency
...
def main():
    ...
    best_baseline_f1 = max(
        metrics["baseline_majority_from_t2_grid"]["macro_f1"],
        metrics["baseline_nearest_neighbor_t2_grid"]["macro_f1"],
    )
    ...
    lines = []
    lines.append("# PROJECT 11 — TRANSFER T2 REPORT")
    lines.append("")
    ...
    lines.append("")
    lines.append("## T2 grid label counts (3x3)")
    lines.append(f"- {metrics['t2_grid_counts']}")
    lines.append("")
    lines.append("## Baselines")
    lines.append(f"- majority label (from T2 grid): `{majority_lbl}`")
    lines.append(f"- majority macro-F1: {metrics['baseline_majority_from_t2_grid']['macro_f1']:.4f}")
    lines.append(f"- nearest-neighbor macro-F1: {metrics['baseline_nearest_neighbor_t2_grid']['macro_f1']:.4f}")
    lines.append("")
