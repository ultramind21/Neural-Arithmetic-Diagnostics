...
def soft_clamp_softplus(x: float, k: float) -> float:
    # matches the audit recompute formulation
...
def macro_f1_fixed3(y_true: List[str], y_pred: List[str]) -> float:
    # macro-F1 across all 3 labels even if some have zero support (F1=0)
    f1s = []
    for lbl in LABELS:
...
def merge_system(hard: dict, soft: dict) -> dict:
    # families + thresholds from hard, k from soft
    out = {}
...
def v31_predict(system: dict, H: float, P: float) -> str:
    # same V3.1 as before (saturation-aware margin expansion)
    sat_risk = 0
    for f in system["families"]:
        base = float(f["base_global"])
        sf = float(f["shared_failure"])
...
def main():
    ...
    # NN predictions for multiple resolutions
    nn_grids = {}
    nn_preds = {}
    ...
    artifact = {
        ...
        "rules": {
            ...
            "V3.1": {
                ...
                "boundary": subset(y_true, y_v31, mask_u),
            }
        },
        "nn": {},
    ...
    # fix boundary subset bug for V3.1
    artifact["rules"]["V3.1"]["boundary"] = subset(y_true, y_v31, mask_b)
