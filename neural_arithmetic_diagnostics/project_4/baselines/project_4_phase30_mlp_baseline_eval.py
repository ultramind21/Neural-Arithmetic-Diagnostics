"""
================================================================================
PROJECT 4 PHASE-30 MLP BASELINE EVALUATION
================================================================================

PURPOSE:
  Execute the first scientifically aligned baseline evaluation path in Project 4
  using the MLPSequenceArithmetic model from phase_30_multidigit_learning.py.

ALIGNMENT PRINCIPLE:
  This file must follow the actual Phase 30 evaluation semantics:
...
IMPORTANT:
  This file no longer attempts a reconstructed full-number exact-match path.
  It uses the native digit/carry evaluation structure consistent with Phase 30.

CURRENT SCOPE:
  - MLP only
  - bounded single-run baseline path
  - Project 4 scorecard integration
  - no final regime assignment
...
def compute_phase30_targets(a: np.ndarray, b: np.ndarray):
    ...
    Returns:
      digit_true: [batch, length]
      carry_true: [batch, length]
    """
    if a.shape != b.shape:
...
# ============================================================================
# PLACEHOLDER DIMENSIONS (EXPLICITLY QUALIFIED)
# ============================================================================
...
def parse_args():
    parser = argparse.ArgumentParser(description="Project 4 Phase30 MLP baseline evaluation")
    parser.add_argument("--model-name", type=str, default="phase30_mlp_baseline")
...
def main():
    ...
    print_header("PROJECT 4 PHASE-30 MLP BASELINE EVALUATION (ALIGNED PATH)")
    ...
    # For Project 4 scorecard projection, map:
    # - in_distribution_accuracy -> exact_match
    # - adversarial pattern values -> exact_match per pattern
    # - length values -> exact_match per tested length
    pattern_breakdown = {
    ...
    scorecard = build_scorecard(
        ...
        tags=["project4", "phase30", "mlp", "aligned_eval_path"],
    )
