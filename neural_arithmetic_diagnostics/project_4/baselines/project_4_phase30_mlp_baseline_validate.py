"""
================================================================================
PROJECT 4 PHASE-30 MLP BASELINE VALIDATION RUNNER
================================================================================

PURPOSE:
  Run repeated Phase-30 MLP baseline evaluations under Project 4 and aggregate
  them into a single validation artifact suitable for stability checking.

ROLE:
  This script:
    - reuses the trained MLP checkpoint
...
BASELINE_EVAL_MODULE = "project_4.baselines.project_4_phase30_mlp_baseline_eval"

RESULTS_DIR = PROJECT_4_ROOT / "results" / "baseline_runs"
VALIDATION_OUTPUT = RESULTS_DIR / "phase30_mlp_validation_runs.json"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Project 4 Phase-30 MLP baseline validation runner")
...
def main():
    ...
    print_header("PROJECT 4 PHASE-30 MLP BASELINE VALIDATION RUNNER")
    ...
    for run_idx in range(args.runs):
        ...
        model_name = f"phase30_mlp_baseline_run{run_idx}"

        command = [
            PYTHON, "-m", BASELINE_EVAL_MODULE,
        ...
        for L in args.lengths:
            command.extend(["--lengths", str(L)])

        for p in args.corruption_levels:
            command.extend(["--corruption-levels", str(p)])
        ...
        if result.returncode != 0:
            ...
            print("STDERR:")
            print(result.stderr)
            raise RuntimeError(f"Validation run {run_idx} failed")
    ...
    validation_payload = {
        "status": "project4_phase30_mlp_validation_runs",
        "checkpoint": args.checkpoint,
        "run_count": args.runs,
        ...
        "notes": [
            ...
            "It aggregates repeated aligned Phase 30 MLP Project 4 baseline runs",
