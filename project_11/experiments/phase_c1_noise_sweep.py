...
OUT_DIR = P11_RESULTS / "phase_c1_noise"
...
def normalized_distance(aH: float, aP: float, bH: float, bP: float) -> float:
    # normalize to Project 10 ranges for stability
...
def main():
    ...
    # Precompute ground truth labels on true inputs
    y_true = []
    kinds = []
    true_HP = []
    for pt in points:
        H = float(pt["H"])
        P = float(pt["P"])
        y_true.append(ground_truth(system, H, P))
        kinds.append(pt.get("kind", "unknown"))
        true_HP.append((H, P))

    # Build NN 11x11 grid labels (same ranges)
    ...
    for sigma_H, sigma_P in NOISE_SETTINGS:
        ...
        for rep in range(REPS):
            rng = random.Random(9000 + rep * 100 + int(sigma_H * 1e6) + int(sigma_P * 1e6))

            y_v3 = []
            y_nn = []

            for (H_true, P_true) in true_HP:
                H_obs = clip(H_true + rng.gauss(0.0, sigma_H), H_MIN, H_MAX)
                P_obs = clip(P_true + rng.gauss(0.0, sigma_P), P_MIN, P_MAX)
    ...
    # Report
    lines = []
    lines.append("# PROJECT 11 — PHASE C1 NOISE SWEEP REPORT")
    lines.append("")
    ...
    lines.append("")
    lines.append("| sigma_H | sigma_P | V3 macro-F1 | NN11 macro-F1 | V3 acc | NN11 acc |")
    lines.append("|---:|---:|---:|---:|---:|---:|")
    for r in results:
        line = (
            f"| {r['sigma_H']:.4f} | {r['sigma_P']:.4f} | "
            f"{r['v3']['macro_f1_mean']:.4f} ± {r['v3']['macro_f1_sd']:.4f} | "
            f"{r['nn11']['macro_f1_mean']:.4f} ± {r['nn11']['macro_f1_sd']:.4f} | "
            f"{r['v3']['acc_mean']:.4f} ± {r['v3']['acc_sd']:.4f} | "
            f"{r['nn11']['acc_mean']:.4f} ± {r['nn11']['acc_sd']:.4f} |"
        )
        lines.append(line)
