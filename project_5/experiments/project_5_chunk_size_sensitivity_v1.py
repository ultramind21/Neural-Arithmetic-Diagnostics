"""
================================================================================
PROJECT 5 CHUNK-SIZE SENSITIVITY V1
================================================================================

PURPOSE:
  Test whether the remaining structural failures after explicit carry-conditioned
  representation depend on the decomposition chunk size.

CORE QUESTION:
  Does changing chunk size alter robustness on:
    - alternating_carry
    - full_propagation_chain
    - block_boundary_stress

CURRENT HYPOTHESIS:
  Some failures may be tied to boundary density or decomposition granularity,
  not only to local carry-conditioned representation itself.
...
JSON_OUTPUT = OUTPUT_DIR / "project_5_chunk_size_sensitivity_v1_artifact.json"
MD_OUTPUT = OUTPUT_DIR / "project_5_chunk_size_sensitivity_v1_report.md"
...
def render_report(artifact: Dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "## Tested Chunk Sizes",
        f"- {artifact['chunk_sizes']}",
        "",
        "## Results",
    ]

    for family, by_chunk in artifact["results"].items():
        lines.append(f"\n### {family}")
        for chunk_size, acc in by_chunk.items():
            lines.append(f"- chunk_size={chunk_size}: {acc}")

    lines.extend([
        "",
        "## Interpretation Boundary",
        "- This experiment tests whether residual failure depends on decomposition granularity.",
...
def main():
    print_header("PROJECT 5 CHUNK-SIZE SENSITIVITY V1")
    ...
    for family_name, (a, b) in families.items():
        ...
        for chunk_size in chunk_sizes:
            results[family_name][str(chunk_size)] = evaluate_family(
                processor, a, b, chunk_size=chunk_size
            )

    artifact = {
        "timestamp_utc": datetime.utcnow().isoformat() + "Z",
        "experiment": "project_5_chunk_size_sensitivity_v1",
        "chunk_sizes": chunk_sizes,
        "results": results,
        "notes": [
            "This experiment tests whether residual failure depends on block granularity.",
            "Explicit carry-conditioned local representation is used as the local processor.",
        ],
    }
