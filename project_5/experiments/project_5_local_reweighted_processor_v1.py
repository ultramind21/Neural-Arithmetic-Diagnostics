"""
================================================================================
PROJECT 5 LOCAL REWEIGHTED PROCESSOR V1
================================================================================

PURPOSE:
  Test whether reweighting local digit learning on carry-producing cases
  improves decomposition performance.

CORE HYPOTHESIS:
  The main bottleneck is not carry prediction itself, but weak digit learning
  on cases with carry_out = 1.

INTERVENTION:
  Train the same local processor architecture, but upweight digit-loss on
  carry-producing local cases.

CURRENT QUESTIONS:
  1. Does local digit accuracy improve?
  2. Does digit accuracy improve specifically on carry_out = 1 cases?
  3. Does blockwise composed performance improve on structured families?
...
JSON_OUTPUT = OUTPUT_DIR / "project_5_local_reweighted_processor_v1_artifact.json"
MD_OUTPUT = OUTPUT_DIR / "project_5_local_reweighted_processor_v1_report.md"
...
def train_local_model_reweighted(device: torch.device):
    ...
    criterion_digit = nn.CrossEntropyLoss(reduction="none")
    criterion_carry = nn.CrossEntropyLoss()

    # Reweight digit-loss for carry_out = 1 cases
    weights = torch.where(carry_t == 1, torch.tensor(2.0, device=device), torch.tensor(1.0, device=device))

    for _ in range(300):
        optimizer.zero_grad()
        digit_logits, carry_logits = model(X_t)

        digit_loss_raw = criterion_digit(digit_logits, digit_t)
        digit_loss = (digit_loss_raw * weights).mean()

        carry_loss = criterion_carry(carry_logits, carry_t)
        loss = digit_loss + carry_loss
        loss.backward()
        optimizer.step()
...
def analyze_local_behavior(X, digit_y, carry_y, digit_pred, carry_pred):
...
def render_report(artifact: Dict[str, Any]) -> str:
    ...
    lines = [
        "# PROJECT 5 LOCAL REWEIGHTED PROCESSOR V1 RESULTS",
        "",
    ...
    lines.extend([
        ...
        "- This experiment tests whether carry-conditioned digit reweighting improves learned decomposition.",
        "- It does not yet establish a final Project 5 solution.",
        "",
    ])
...
def main():
    print_header("PROJECT 5 LOCAL REWEIGHTED PROCESSOR V1")
    ...
    model, X, digit_y, carry_y, digit_pred, carry_pred = train_local_model_reweighted(device=device)
    processor = LearnedLocalProcessor(model=model, device=device)

    print("✓ Reweighted local processor trained")

    local_analysis = analyze_local_behavior(X, digit_y, carry_y, digit_pred, carry_pred)
    ...
    artifact = {
        ...
        "experiment": "project_5_local_reweighted_processor_v1",
        ...
        "results": results,
        "notes": [
            "This experiment upweights digit-loss on carry-producing local cases.",
            "Its purpose is to test whether the carry-conditioned digit bottleneck can be reduced.",
        ],
    }
